...
'''
TRAIN_BATCH_SZ = 60   # Batch size for train / test sets
TEST_BATCH_SZ = 100
MAX_X_VALUE = 100   # Maximum value for x for y = x^2
...
DIM_H2 = 8  # 2st hidden layer dimension
# Output dimension           (One-hot classification [y < x^2, y = x^2, y > x^2])
DIM_OUT = 2
...
def train(net, train_set):
    ...
    while epoch < EPOCHS and loss.item() > CUTOFF_LOSS:
        ...
        for data in train_set:
            ...
            # Calculate how incorrect the NN was
            loss = F.mse_loss(output[0], label)
        ...
        if epoch % 1 == 0:
            print(
                f'Epoch #{str(epoch).ljust(2)} loss: {round(loss.item(), 3)}')
...
def generate_set(batch_size):
    ...
    for i in range(0, batch_size):
        ...
        x = float(random.randrange(MAX_X_VALUE * -1 /
                                   RAND_STEP, MAX_X_VALUE / RAND_STEP))
        x = x * RAND_STEP

        deltaY = DELTA_Y_VALUE - MIN_Y_DELTA

        y = float(random.randrange(deltaY * -1 / RAND_STEP,
                                   deltaY / RAND_STEP)) + MIN_Y_DELTA
        y = ((y * RAND_STEP) + x) ** 2
...
def print_set(set, batch_size):
    ...
    for i in range(batch_size):
        ...
        print(
            f'{str(round(sample[0].item(), 3)).ljust(8)} | {str(round(sample[0].item()**2,3)).ljust(8)} | {str(round(sample[1].item(), 3)).ljust(8)} | {label_str}')
